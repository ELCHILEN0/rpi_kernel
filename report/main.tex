...
These initial examples were designed to demonstrate the performance penalty of poor cache utilization in user programs. \textbf{TODO: reword intro}  The first example will operate on 100\% of the cache while the second will use 400\% thus requiring a full cache refill for each iteration (with some degree of randomness relating to the replacment policy).  These examples serve as important baselines for demonstrating predicable behaivour when scaling a problem up to multiple cores; having a 4 times larger cycle count and nearly 900\% larger refill rate when the L1 cache is consistently overfilled.  This results in frequent thrashing of kernel memory leading to roughly 11\% less IPC.  For the next example, the highly utilized ...
...
% Extending the previous examples to a multicore system serves as a demonstration to the benefit of well thought out parallelization.  A single sample operating on 400\% of a processors cache at hard difficulty was executed against the global scheduler and the core scheduler with process affinity to separate computation onto each of the four cores.  Despite having a 94\% APR improvement in userspace, the difference in IPC is nearly insignificant.  However, in kernel execution there is a 76\% improvement in cycles which results in a cumulative 20\% improvment in kernel IPC.  This is likely due to predictable cache access patterns leading to fewer kernel refills and wasted cycles.  Additionally, the number of kernel instructions is 47\% less when running with a per core runqueue, owing to no lock contention between cores at each a context switch.

Extending the previous examples to a multicore system serves as a demonstartion to the benefit of well thought out parallelization.  A single sample executing on 400\% of a processors cache at hard difficulty was executed against the global scheduler and the core scheduler with fixed process affinity to separate computation onto each of the four cores.  Even without comparing the difference between the two schedulers, the global queue outperforms the single core example working on a matrix of the same size.  This was particularily interesting since it is not inherently obvious what may be causing this superliner speedup.  

However, this is likely a result of data being precached in the L2 cache by the other processors.  As a result of striding the computation, the innermost edges of data will exhibit high spatial locality despite having low temporal locality.  However, having low temporal locality effectively acts as a form of prefetching for subsequent accesses.  This prefetching results in data being present in the L2 cache well before its equivalent access to main memory would be made.  Since the single core example does not perform inner striding, these performance gains cannot be made.  Although, the stride length was chosen to be quite large for these examples, decreasing the stride length would allow more data to be prefetched in the L2 cache depending on scheduling and process execution order.
