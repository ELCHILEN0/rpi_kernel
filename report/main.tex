...
Areas of interest that do not nesecarily involve much design modification might include migrating userspace to EL0 and implementing user virtual address space.  This would likely add another level of performance comparision on context switching related to reloading the TLB and possible cache invalidation on MMU-less architectures.

With regards to process migration, individual proceses could be benchmarked by the kernel and this could be used to improve execution by dtermining a scheduling which minimizes thrashing.  Additionally, impementing variable quantum size could provide variable quantum as a method of adjusting execution fairness.
