...
% DONE
Extending the previous examples to a multicore system demonstrates the benefit of well thought out parallelization.  A large matrix is separated into four medium strided matrices and will execute at hard difficulty.  They will be executed against both the global scheduler and the core scheduler with fixed process affinity to separate computation onto each of the four cores.  Without comparing the differences between the two schedulers, they both outperform the single core computation on the same problem size.  Surprisingly, both exhibit superlinear speedup.
...
If I were to continue with this project, there are a couple aspects which would be worth exploring. It would be interesting to implement communication routines and implementing a microkernel architecture and comparing some of the differences between the two architectures.  Additionally various forms of asymetric multiprocessing would be interesting to learn, especially with regards to measuring other performance metrics.  For example, instead of focusing on total throughput, power might be an important aspect to focus on, utilizing lower power \texttt{WFE/SEV} combination to sleep processors.  Since a master-slave kernel ``preserves the uniprocessor execution environment from the kernel's point of view'' and thus can ``run on an SMP system without race conditions'', comparing the kernel performance to that of a spin-locked kernel would be interesting.

Areas of interest that do not nesecarily involve much design modification might include migrating userspace to EL0 and implementing user virtual address space.  This would likely add another level of performance comparision on context switching related to reloading the TLB and possible cache invalidation on MMU-less architectures.  Additionally, implementing further POSIX compliancy such as file I/O would allow tests ot be run on more interesting programs.  For example, the design of applications which stream data over a network
