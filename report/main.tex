...
The examples serve as baselines for comparing behaivour when scaling the problem to multiple cores.  The first example will operate on a small matrix while the second uses a large requiring a full cache refill on every iteration (with some degree of randomness relating to the replacement policy).  On the second data set, the results are predictably much larger due to frequent data thrashing.  However, despite a large difference in cache access per refill (APR), the total relative runtime seems to be fairly independent.

\textbf{TODO: more here? maybe not its not too relavent}
